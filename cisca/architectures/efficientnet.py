...
def _efficientnet(input_shape, blocks_args_list, global_params, block_lookup):
    ...
    for numblock, blocks_args in enumerate(blocks_args_list):
        print(blocks_args.num_repeat)
        ...
        x = MBConvBlock(blocks_args, global_params, idx, idblock, drop_connect_rate=drop_rate_dx * idx)(x)
        block_lookup[idx] = idblock
        idx += 1
...
def get_model_by_name(model_name, input_shape, classes=1000, pretrained=False):
    ...
    block_lookup = {}
    model = _efficientnet(input_shape, blocks_args, global_params, block_lookup)
    print(block_lookup)

...
def get_efficientnet_params(model_name, override_params=None):
    ...
    params_dict = {
        # (width_coefficient, depth_coefficient, magnification, dropout_rate)
        # Note: the magnification here is just for reference, its values won't be used.
...
def MBConvBlock(block_args, global_params, idx, idblock, drop_connect_rate=None):
    filters = block_args.input_filters * block_args.expand_ratio
    batch_norm_momentum = global_params.batch_norm_momentum
    batch_norm_epsilon = global_params.batch_norm_epsilon
    has_se = (block_args.se_ratio is not None) and (0 < block_args.se_ratio <= 1)

    block_name = 'blocks_' + str(idblock) + '_' + str(idx) + '_'

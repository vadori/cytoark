...
parser = argparse.ArgumentParser(description="Oversampling Pannuke dataset",
...
# -df "D:/NucleiSegmentation/Projects/cytoark/datasets/Pannuke/40x/256x256"-vdf "D:/NucleiSegmentation/Projects/cytoark/datasets/Pannuke/40x/256x256"
# data_folder = "D:/NucleiSegmentation/Projects/cytoark/datasets/Pannuke/40x/256x256" 
# valid_data_folder = "D:/NucleiSegmentation/Projects/cytoark/datasets/Pannuke/40x/256x256"
...
for img_id in tqdm(partition[0]):
    ...
    count = np.bincount([int(rp[i].intensity_max) for i in np.arange(len(rp))], minlength=6).reshape((1,6))
    counts = np.append(counts, count, axis=0)

## debugging 
# img_id = partition[0][20]
# img = cv2.cvtColor(cv2.imread(os.path.join(image_folder, "{0}.png".format(img_id)), cv2.IMREAD_COLOR),cv2.COLOR_BGR2RGB)  
# labelmap = cv2.imread(os.path.join(instance_mask_folder, "{0}.tiff".format(img_id)), -1)  
# cell_class_mask = cv2.imread(os.path.join(class_mask_folder, "{0}.tiff".format(img_id)), -1) 
...
n_extras = (n_extras * [1,1,1,1]).astype(int)
...
for c, n_extra in zip(extra_classes, n_extras):
    ...
    prob = np.clip(dftrain["prob"].values, 0, np.percentile(dftrain["prob"].values, 98.5)) # was 99.8
    #prob = prob ** 2
...
# python oversample_pannuke3.py -df "D:\NucleiSegmentation\Projects\cytoark\datasets\Pannuke\40x\256x256" -vdf "D:\NucleiSegmentation\Projects\cytoark\datasets\Pannuke\40x\256x256"
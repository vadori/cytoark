...
SEED = 5
info = pd.read_csv('D:/NucleiSegmentation/Projects/cytoark/datasets/pannuke/40x/256x256/pannuke_info.csv')
file_names = np.squeeze(info["name"].to_numpy()).tolist()

img_sources = np.unique(file_names)
# img_sources = [v.rsplit('_',1)[0] for v in file_names]
# img_sources = np.unique(img_sources)
cohort_sources = [v.rsplit('_',2)[0] for v in img_sources]
...
img_sources = np.unique(traindata)
# img_sources = [v.rsplit('_',1)[0] for v in file_names]
# img_sources = np.unique(img_sources)
cohort_sources = [v.rsplit('_',2)[0] for v in img_sources]
_, cohort_sources = np.unique(cohort_sources, return_inverse=True)
